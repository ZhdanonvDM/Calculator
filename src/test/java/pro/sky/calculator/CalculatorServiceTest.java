...
private String a, b, c, d;
    ...
    void plus() throws Exception {
     assertEquals("5 + 8 = 13", cs.plus(a,b));
     assertEquals("-5 + -8 = -13", cs.plus(c,d));
    }

    @Test
    void minus() throws Exception {
        assertEquals("5 - 8 = -3",cs.minus(a, b));
        assertEquals("-5 - -8 = 3",cs.minus(c,d));
    }

    @Test
    void multiply() throws Exception {
        assertEquals("5 * 8 = 40",cs.multiply(a, b));
        assertEquals("-5 * -8 = 40",cs.multiply(c, d));
    }

    @Test
    void divide() throws Exception {
        assertEquals("5 / 8 = 0",cs.divide(a, b));
        assertEquals("-5 / -8 = 0",cs.divide(c, d));
        assertThrows(IllegalArgumentException.class, () -> cs.divide(a, "0"));
    }

    @ParameterizedTest
    @MethodSource("dataTestPlus")
    public void testPlus (String n1, String n2, String expected) throws Exception {
        assertEquals(expected,cs.plus(n1, n2));
    }
    @ParameterizedTest
    @MethodSource("dataTestMinus")
    public void testMinus (String n1, String n2, String expected) throws Exception {
        assertEquals(expected,cs.minus(n1, n2));
    }

    @ParameterizedTest
    @MethodSource("dataTestMultiply")
    public void testMultiply (String n1, String n2, String expected) throws Exception {
        assertEquals(expected,cs.multiply(n1, n2));
    }

    @ParameterizedTest
    @MethodSource("dataTestDevide")
    public void testDevide (String n1, String n2, String expected) throws Exception {
